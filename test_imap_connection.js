 ...
 *
 * This script tests your IMAP configuration to ensure you can successfully
 * connect to Gmail's IMAP server and receive emails.
 *
 * Usage:
 *   node test_imap_connection.js
 *
 * Prerequisites:
 * - IMAP_USER and IMAP_PASSWORD must be set in .env file
...
/**
 * Step 2: Test IMAP Connection and Authentication
 */
async function testIMAPConnection() {
    logSubsection('IMAP Connection & Authentication');
    ...
    try {
        ...
        const config = {
            imap: {
                ...
                tls: true,
                authTimeout: 5000, // 5 second timeout
            }
        };
...
async function runTests() {
    logSection('IMAP Connection Test');
    log(`Started: ${new Date().toLocaleString()}`, 'cyan');

    // Step 1: Environment Variables
    validateEnvironment();

    // Step 2: IMAP Connection & Authentication
