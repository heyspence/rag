/**
 * Shared tools module for both LM Studio and HTTP MCP servers
 * Contains all tool definitions and implementations to avoid duplication
 */
...
/**
 * Get the complete list of available tools with their schemas
 */
...
async function handleToolCall(toolName, args, dependencies) {
    const { vectorDb, embeddingEngine, emailService } = dependencies;
            ...
            try {
                ...
                if (filteredResults.length === 0) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `No documents found meeting the minimum similarity score of 0.7.`,
                            },
                        ],
                    };
                }
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Top ${filteredResults.length} relevant results (Threshold: 0.7):\n\n${formattedResults}`,
                        },
                    ],
                };
            } catch (error) {
